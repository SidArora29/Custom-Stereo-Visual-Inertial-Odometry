...
Version 4.7: Perfect physical gravity alignment via initial pose orientation [1].
"""
...
# 1. Align Initial Pose to the true gravity vector [1]
...
# CRITICAL FIX: Align the initial orientation with gravity [1]
# This defines world Z as gravity-vertical, while keeping starting coordinates at (0,0,0)
initial_pose = gtsam.Pose3(gtsam.Rot3(R_imu_world.T), gtsam.Point3(0, 0, 0))

# 2. Calculate initial biases (Gyro gets stationary mean, Accel gets scale offset) [1]
...
# Gravity is set to default standard gravity pointing straight down [1]
...
stereo_matcher = cv2.StereoSGBM_create(
    minDisparity=0,
    numDisparities=64,   # max disparity pixels
    blockSize=5,
    P1=8  * 3 * 5**2,
    P2=32 * 3 * 5**2,
    disp12MaxDiff=1,
...
# Add after global_lm_counter setup
lm_age = {}   # lm_id -> number of frames it has been observed

MAX_LM_AGE = 5   # drop landmarks after this many frames
        ...
        if p1 is not None and st_l is not None:
            ...
            if len(good_l) > 0:
                ...
                for i in range(len(good_l)):
                    ...
                    Z = Tx / disp          
                    if Z < 0.5 or Z > 30.0 or not np.isfinite(Z): 
                        continue
                    ...
                    if lm_id not in added_landmarks:
                        ...
                        # Weak landmark depth prior to ensure Hessian conditioning [1]
...
# =================================================================
    # START OF THE CLEANED-UP REPLENISHMENT BLOCK
    # =================================================================
    
    # 1. Compute dropped features BEFORE updating our tracking lists
    dropped_ids = set(current_lm_ids) - set(surviving_ids)

    # 2. Safely update state for the next frame
    ...
    # 3. Replenish features if the tracker is running low
    if len(p0) < 150:
    ...
    # 4. Queue dropped landmarks for ISAM2 marginalization
    # keys_to_drop = gtsam.KeyVector()
    # for lm_id in dropped_ids:
    #     if lm_id in added_landmarks:
    #         keys_to_drop.append(L(lm_id))
    #         added_landmarks.discard(lm_id)

    # =================================================================
    # END OF CLEANUP. Continue directly to STEP D.
    # =================================================================
    ...
    #     try:
    #         isam.marginalizeLeaves(keys_to_drop)
    #     except RuntimeError:
    #         pass
...
# Around Line 420 (Bottom of the loop):
    
    # Explicitly delete the massive dense disparity array from RAM
    if 'disp_map' in locals():
        del disp_map

    tum_rows.append(f"{current_cam_time:.9f} {p[0]:.6f} {p[1]:.6f} {p[2]:.6f} "
                    f"{qx:.6f} {qy:.6f} {qz:.6f} {qw:.6f}")
